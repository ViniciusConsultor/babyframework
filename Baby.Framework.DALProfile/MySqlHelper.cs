...
namespace Baby.Framework.DALProfile
{
    public sealed class MySqlHelper
    {
        ...
        public static readonly string ConnectionStringLocal = ConfigurationManager.ConnectionStrings["MYSQL_DB"].ConnectionString;
        ...
        /// <param name="commandParams">An array of SqlParamters used to execute the command</param>
        /// <returns>An int representing the number of rows affected by the command</returns>
        public static int ExecuteNonQuery(CommandType commandType, string commandText, params MySqlParameter[] commandParams)
        {
            return ExecuteNonQuery(ConnectionStringLocal, commandType, commandText, commandParams);
        }
        ...
        public static int ExecuteNonQuery(string connectionString, CommandType commandType, string commandText, params MySqlParameter[] commandParameters)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                // Call the overload that takes a connection in place of the connection stringusp_Bstudents_Update
        ...
        /// is created, opened, and closed during this method.(With the default connection)
        /// </summary>
        /// <param name="commandType">The CommandType (stored procedure, text, etc.)</param>
        ...
        public static DataRow ExecuteDataRow(CommandType commandType, string commandText, params MySqlParameter[] parms)
        {
            return ExecuteDataRow(ConnectionStringLocal, commandType, commandText, parms);
        }
        ...
        /// using the provided parameters.(With the default connection)
        /// </summary>
        /// <remarks>
        /// e.g.:  
        ...
        {
            return ExecuteDataset(ConnectionStringLocal, commandType, commandText, commandParameters);
        }
        ...
        /// <summary>
        /// Executes a single command against a MySQL database.(With the default connection)
        /// </summary>
        /// <param name="commandType">The CommandType (stored procedure, text, etc.)</param>
        ...
        {
            MySqlConnection conn = new MySqlConnection(ConnectionStringLocal);
            MySqlCommand cmd = new MySqlCommand();
            try
            {
                bool mustClose = true;
                PrepareCommand(cmd, conn, null, commandType, commandText, commandParameters, out mustClose);
                MySqlDataReader mysqlReader = cmd.ExecuteReader(CommandBehavior.CloseConnection);
                cmd.Parameters.Clear();
                return mysqlReader;
            }
            catch
        ...
        /// <summary>
        /// Execute a single command against a MySQL database.(With the default connection)
        /// </summary>
        /// <param name="commandType">The CommandType (stored procedure, text, etc.)</param>
        /// <param name="commandText">Command text to use for the update</param>
        /// <param name="commandParameters">Parameters to use for the command</param>
        ...
        /// <param name="commandParameters">Parameters to use for the command</param>
        /// <returns>The first column of the first row in the result set, or a null reference if the result set is empty.</returns>
        public static object ExecuteScalar(string connectionString, CommandType cmdType, string commandText, params MySqlParameter[] commandParameters)
        {
            ...
            {
                bool mustClose = true;
                PrepareCommand(mysqlCmd, cn, null, cmdType, commandText, commandParameters, out mustClose);
                retval = mysqlCmd.ExecuteScalar();
